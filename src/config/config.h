...
public:
    ...
    template <class Ty_v> inline Ty_v get(const std::string& key, const Ty_v& fallback) const
    {
        return _yaml[key].as<Ty_v>(fallback);
    }
    inline std::string get(const std::string& key, const char* fallback) const
    {
