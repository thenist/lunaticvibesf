...
bool InputDirectInput8::releaseDevices()
{
    ...
    if (deviceMouse.lpdid)
        deviceMouse.lpdid->Release();

    if (deviceKeyboard.lpdid)
        deviceKeyboard.lpdid->Release();
...
int InputDirectInput8::refreshDevices()
{
    ...
    // create joysticks
    if (HRESULT hres =
            lpdi->EnumDevices(DI8DEVCLASS_GAMECTRL, WDIEnumDevicesCallbackJoystick, (LPVOID)this, DIEDFL_ATTACHEDONLY);
        hres == DI_OK)
    {
