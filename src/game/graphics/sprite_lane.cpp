...
SpriteLaneVertical::SpriteLaneVertical(const SpriteLaneVerticalBuilder& builder) : SpriteStatic(builder)
{
    ...
    if (_category != chart::NoteLaneCategory::EXTRA)
    {
        ...
        case PLAYER_SLOT_PLAYER:
            if ((_index == chart::NoteLaneIndex::Sc1 &&
                 (gPlayContext.mods[PLAYER_SLOT_PLAYER].assist_mask & PLAY_MOD_ASSIST_AUTOSCR)) ||
        ...
        case PLAYER_SLOT_TARGET:
            if (_index == chart::NoteLaneIndex::Sc2 &&
                (gPlayContext.mods[gPlayContext.isBattle ? PLAYER_SLOT_TARGET : PLAYER_SLOT_PLAYER].assist_mask &
                 PLAY_MOD_ASSIST_AUTOSCR))
            {
                if (!_autoNotes)
                    hideInternal = true;
            }
...
void SpriteLaneVertical::draw() const
{
    ...
    if (pNote && pNote->pTexture && pNote->pTexture->isLoaded())
    {
        ...
        {
            pNote->pTexture->draw(
                pNote->textureRects[pNote->selectionIndex * pNote->animationFrames + pNote->animationFrameIndex], r,
    ...
    {
        _hiddenCompatibleTexture->draw(RECT_FULL, _hiddenCompatibleArea, Color(0xffffffff), BlendMode::ALPHA, false, 0);
    }
}
...
void SpriteLaneVerticalLN::setMotionStartTimer(IndexTimer t)
{
    ...
    case IndexTimer::K29_LN_BOMB: animLimited = true; break;

    default: animLimited = false; break;
    }
}
        ...
        while (!pChart->isLastNote(_category, _index, it) && head_y_actual >= 0)
        {
            ...
            if (it->flags & Note::LN_TAIL)
            {
                ...
                if (tailMetreOffset >= 0)
                    ...
                    tail_y = (c.y + c.h) -
                             static_cast<int>(std::floor(-tailMetreOffset * _noteAreaHeight * _basespd * _hispeed));
            ...
            else
            {
                ...
                head_y_actual = (c.y + c.h) -
                                static_cast<int>(std::floor(-headMetreOffset * _noteAreaHeight * _basespd * _hispeed));
                if (head_y_actual < head_y)
                    head_y = head_y_actual;
    ...
    else
    {
        ...
        while (!pChart->isLastNote(_category, _index, it) && head_y_actual >= 0)
        {
            ...
            if (it->flags & Note::LN_TAIL)
            {
                ...
                tail_y = (c.y + c.h) -
                         static_cast<int>(std::floor(-tailTimeOffset / 1600.0 * _noteAreaHeight * _basespd * _hispeed));
            ...
            else
            {
                ...
                head_y_actual =
                    (c.y + c.h) -
                    static_cast<int>(std::floor(-headTimeOffset / 1600.0 * _noteAreaHeight * _basespd * _hispeed));
                if (head_y_actual < head_y)
                    head_y = head_y_actual;
...
void SpriteLaneVerticalLN::draw() const
{
        ...
        for (auto it = _outRectBody.begin(); it != _outRectBody.end(); ++it)
        {
            ...
            if (itNext == _outRectBody.end())
            {
                pNoteBody->pTexture->draw(
                    pNoteBody->textureRects[pNoteBody->selectionIndex * pNoteBody->animationFrames +
                                            pNoteBody->animationFrameIndex],
                    *it, miss ? colorMiss : _current.color, miss ? BlendMode::ALPHA : _current.blend, _current.filter,
                    _current.angle, _current.center);
            }
            else
            {
                pNoteBody->pTexture->draw(pNoteBody->textureRects[pNoteBody->selectionIndex], *it,
                                          miss ? colorMiss : _current.color, miss ? BlendMode::ALPHA : _current.blend,
                                          _current.filter, _current.angle, _current.center);
            }
        }
    }
    ...
    if (pNote && pNote->pTexture && pNote->pTexture->isLoaded())
    {
        for (auto it = _outRect.begin(); it != _outRect.end(); ++it)
        {
            ...
            {
                pNote->pTexture->draw(pNote->textureRects[pNote->selectionIndex], *it,
                                      miss ? colorMiss : _current.color, miss ? BlendMode::ALPHA : _current.blend,
                                      _current.filter, _current.angle, _current.center);
            }
        }
        ...
        for (auto it = _outRectTail.begin(); it != _outRectTail.end(); ++it)
        {
            ...
            if (itNext == _outRectTail.end())
            {
                pNoteTail->pTexture->draw(
                    pNoteTail->textureRects[pNoteTail->selectionIndex * pNoteTail->animationFrames +
                                            pNoteTail->animationFrameIndex],
                    *it, miss ? colorMiss : _current.color, miss ? BlendMode::ALPHA : _current.blend, _current.filter,
                    _current.angle, _current.center);
            }
            else
            {
                pNoteTail->pTexture->draw(pNoteTail->textureRects[pNoteTail->selectionIndex], *it,
                                          miss ? colorMiss : _current.color, miss ? BlendMode::ALPHA : _current.blend,
                                          _current.filter, _current.angle, _current.center);
            }
        }
